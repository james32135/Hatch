...
export default function ChildPortfolio() {
  ...
  const totalUsd = p.data?.latestSnapshot?.totalUsd ?? p.data?.totalUsd;
  const holdings = p.data?.holdings || [];
  const allocation = p.data?.allocation || {};
  const alloc = Object.entries(allocation).map(([k, v]: any) => ({ name: k, value: Number(v) }));
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      <SectionCard className="lg:col-span-2" title="Value" subtitle={sodexError ? "SoDEX returned an error — showing last snapshot" : undefined}>
        {p.isLoading ? <div className="text-sm text-white/50">Loading…</div> : totalUsd == null ? <Unavailable /> : (
          <>
            ...
            {p.data?.performance?.dayChangePct != null && (
              <div className={`mt-1 text-sm ${Number(p.data.performance.dayChangePct) >= 0 ? "text-[hsl(142_71%_55%)]" : "text-[hsl(350_89%_65%)]"}`}>
                {fmtPct(p.data.performance.dayChangePct, { sign: true })} today
              </div>
            )}
              ...
              <tbody>
                {holdings.map((h: any, i: number) => (
                  <tr key={i} className="border-t border-white/5">
                    ...
                    <td className="py-2 text-right font-mono">{h.balance ?? h.amount ?? "—"}</td>
      ...
      <SectionCard className="lg:col-span-3" title="Transactions">
        {(tx.data?.transactions || []).length === 0 ? <div className="text-sm text-white/50">No transactions yet.</div> : (
          <div className="space-y-1">
            {(tx.data?.transactions || []).slice(0, 10).map((t: any, i: number) => (
              <div key={i} className="flex items-center justify-between rounded border border-white/5 bg-white/[0.02] px-3 py-2 text-xs">
                <div><span className="font-mono">{t.kind || t.type}</span> <span className="text-white/40">{t.symbol || t.market || ""}</span></div>
                <div className="flex items-center gap-3"><span className="font-mono text-white/70">{fmtUsd(t.notionalUsd ?? t.amountUsd)}</span><span className="text-white/40">{fmtDate(t.at || t.createdAt)}</span></div>
              </div>
            ))}
          </div>
        )}
